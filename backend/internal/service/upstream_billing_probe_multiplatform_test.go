...
func TestUpstreamBillingProbeGrokAccountPersistsSnapshot(t *testing.T) {
	...
	require.Equal(t, "https://relay.example/v1/sub2api/upstream-info", upstream.lastReq.URL.String())
...
func TestUpstreamBillingProbeAdaptiveCNOfficialBaseURLIsUnsupportedWithoutRequest(t *testing.T) {
	for _, platform := range []string{PlatformKimi, PlatformZhipu, PlatformDeepseek} {
		t.Run(platform, func(t *testing.T) {
			...
			account.Credentials["api_base_urls"] = map[string]any{
				APIProtocolChatCompletions: "  " + account.defaultCNProtocolBaseURL(APIProtocolChatCompletions) + "  ",
			}
			repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{account.ID: account}}
			upstream := &httpUpstreamRecorder{}
			svc := newUpstreamBillingProbeTestService(repo, upstream, &upstreamBillingProbeSettingRepo{})
...
func TestUpstreamBillingProbeOpenAIDefaultBaseURLPreserved(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "https://api.openai.com/v1/sub2api/upstream-info", upstream.lastReq.URL.String())
