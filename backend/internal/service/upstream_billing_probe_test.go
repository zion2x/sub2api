...
func TestUpstreamBillingProbeAdaptiveCNUsesChatProtocolBaseURL(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, UpstreamBillingProbeStatusOK, snapshot.Status)
	require.Equal(t, "https://chat-relay.example/v1/sub2api/upstream-info", upstream.lastReq.URL.String())
}
...
func TestUpstreamBillingProbeUnsupportedAndAccountToggle(t *testing.T) {
	...
	require.Equal(t, fixedNow.Add(8*time.Minute), snapshot.NextProbeAt)
